...
export default function Home() {
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-950 text-white overflow-hidden">
      ...
      <div className="flex gap-2 px-3 pb-2 shrink-0 overflow-x-auto">
        {CATEGORIES.map((c, i) => (
          <button
            key={c.label}
        ...
        <div className="px-3 pb-2 shrink-0">
          <div className="flex items-center gap-2 bg-gray-900 rounded-xl px-3 py-2">
            <div className="flex-1 flex flex-wrap gap-1">
              {selected.map(q => (
                <span key={q} className="bg-green-800 text-xs px-2 py-1 rounded-full flex items-center gap-1 whitespace-nowrap">
                  {q}
            ...
            {selected.length >= 2 && (
              <button
                ...
                className="shrink-0 bg-green-600 px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap"
              >
                📊 比較
              </button>
            )}
          ...
          {cat.queries.map(q => {
            ...
            return (
              ...
              >
                <button
                  onClick={() => toggle(q)}
                  className="w-14 h-14 flex items-center justify-center shrink-0 text-xl font-bold text-gray-300"
                >
                  {isSelected ? '✓' : '+'}
                </button>
                <button
                  onClick={() => router.push(`/food/${encodeURIComponent(q)}`)}
                  className="flex-1 text-left py-4 text-base font-medium whitespace-nowrap"
                >
                  {q}
                </button>
                <span className="text-gray-500 text-sm pr-4">→</span>
              </div>
            )
          })}
        </div>
